...
for architecture in "${architectures[@]}"; do
  work_dir="$architecture/$rootfsDir"
  # rm -rf $work_dir ${architecture}.${distro}.tar.xz || true
...
EOF
  ...
  if [ $distro = "kali-last-snapshot" ]; then
    cat $work_dir/usr/lib/os-release | awk -F= '$1=="VERSION" { print $2 ;}' | tr -d '"' > release.version
  fi
